...
[13] HyperMindAgent: Natural Language Query with LLM...
    ...
    AGENT ANSWER:
    ------------------------------------------------------------
    The server room is classified as a "Secure Area" and has properties
    including "Temperature Control" and "Access Control."
    ------------------------------------------------------------
    ...
    WITH kg AS (
      SELECT * FROM graph_search('
        ...
        SELECT ?property ?value ?type WHERE {
          ...
          ?serverRoom rdf:type ?type .
          FILTER(?type = iot:ServerRoom)
        }
      ')
    ) SELECT * FROM kg
    ```
    sparql_inside_cte:
    SELECT ?property ?value ?type WHERE { ?serverRoom ?property ?value ... }

    PROOF HASH: SHA-256 8b03c302033b9fbd...
...
3. **SQL with CTE Generation** → LLM generates SQL with `graph_search()` CTE (universal format)
