...
class SPARQLService: ObservableObject {
    ...
    func executeQuery(_ query: String = "SELECT ?product ?name ?price WHERE { ?product a schema:Product . ?product schema:name ?name . ?product schema:price ?price } LIMIT 20") async throws -> [String] {
