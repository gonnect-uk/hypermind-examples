...
fn test_delete_data_verify_deletion() {
    ...
    assert_eq!(age_quads[0].predicate, OwnedNodeData::from_node(&age));
}
...
fn test_insert_where_with_filter() {
    ...
    assert!(bob_quads.iter().all(|q| q.predicate == OwnedNodeData::from_node(&age)));
}
...
fn test_insert_where_with_optional() {
    ...
    let alice_contact = find_by_subject(&store, &alice)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&contact))
        .unwrap();
...
fn test_insert_where_variable_binding() {
    ...
    let prod1_tax = find_by_subject(&store, &prod1)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&tax))
        .unwrap();
...
fn test_insert_where_join_pattern() {
    ...
    let alice_knows: Vec<OwnedQuadData> = find_by_subject(&store, &alice)
        .into_iter()
        .filter(|owned| owned.predicate == OwnedNodeData::from_node(&knows))
        .collect();
...
fn test_insert_where_union() {
    ...
    let alice_display = find_by_subject(&store, &alice)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&display))
        .unwrap();
...
fn test_insert_where_construct_pattern() {
    ...
    let first_owned = find_by_subject(&store, &alice)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&first))
        .unwrap();
    let last_owned = find_by_subject(&store, &alice)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&last))
        .unwrap();
    ...
    let full_owned = find_by_subject(&store, &alice)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&full))
        .unwrap();
...
fn test_insert_where_aggregation() {
    ...
    for item_owned in items {
        ...
        if let Some(qty_owned) = find_by_subject(&store, &item)
            .into_iter()
            .find(|owned| owned.predicate == OwnedNodeData::from_node(&quantity)) {
    ...
    let total_owned = find_by_subject(&store, &order)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&total))
        .unwrap();
...
fn test_insert_where_subquery() {
    ...
    let members: Vec<OwnedQuadData> = find_by_subject(&store, &team)
        .into_iter()
        .filter(|owned| owned.predicate == OwnedNodeData::from_node(&member))
        .collect();
    ...
    for owned in members {
        ...
        let score_quads: Vec<OwnedQuadData> = find_by_subject(&store, &mem)
            .into_iter()
            .filter(|owned| owned.predicate == OwnedNodeData::from_node(&score))
            .collect();
    ...
    let top_owned = find_by_subject(&store, &team)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&top_performer))
        .unwrap();
...
fn test_modify_transactional_semantics() {
    ...
    let version_owned = find_by_subject(&store, &doc)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&version))
        .unwrap();
    ...
    let status_owned = find_by_subject(&store, &doc)
        .into_iter()
        .find(|owned| owned.predicate == OwnedNodeData::from_node(&status))
        .unwrap();
