...
└─────────────────────┘           └─────────────────────┘
    ...
    WITH kg AS (SELECT * FROM graph_search('...'))
    SELECT * FROM kg
```
...
```
[15] HyperMindAgent: Natural Language Query with LLM...
    KG-grounded recommendations found: 5
    Agent: music-advisor
    Model: GPT-4o
    ...
    KG EVIDENCE (from SPARQL):
      ...
      - Guns N' Roses: undefined
      - Black Sabbath: undefined
      - Led Zeppelin IV: undefined

    AGENT ANSWER:
    ------------------------------------------------------------
    Similar artists to Metallica are Slayer and Megadeth, while a similar
    artist to Led Zeppelin is Deep Purple.
    ------------------------------------------------------------
    ...
    WITH kg AS (
      SELECT * FROM graph_search('
        ...
        SELECT ?artist ?name WHERE {
          ...
          UNION
          { music:Metallica music:genre ?genre }
          FILTER(?artist != music:LedZeppelin && ?artist != music:Metallica)
        }
      ')
    ) SELECT * FROM kg
    ```

    PROOF HASH: SHA-256 3fb38fa457670090...
...
3. **SQL with CTE Generation** → LLM generates SQL with graph_search() for influence chains and genre relations
4. **Query Execution** → Rust executes SPARQL inside graph_search() via NAPI-RS
